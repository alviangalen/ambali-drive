...
// Helper to log audit
const logAudit = async (adminId: string, action: string, targetId?: string, details?: string, ipAddress?: string) => {
  try {
...
router.put('/users/:id/block', async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    const { id } = req.params;
    ...
    await logAudit(
      ...
      user.id, 
      `User ${user.email} was ${isBlocked ? 'blocked' : 'unblocked'}`, 
      req.ip
    );
...
// PUT /change-password
router.put('/change-password', async (req: AuthRequest, res: Response): Promise<any> => {
  try {
